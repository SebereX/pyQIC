...
def calculate_r2(self, no_rc = False, no_interp = False):
    ...
    normalizer = 1 / trapz(np.append(self.d_l_d_varphi,self.d_l_d_varphi[0]), varphi_ext)
    self.B20_mean = trapz(np.append(B20 * self.d_l_d_varphi, B20[0] * self.d_l_d_varphi[0]), varphi_ext) * normalizer
    # Variation in B20
    self.B20_anomaly = B20 - self.B20_mean
    temp = (B20 - self.B20_mean) * (B20 - self.B20_mean) * self.d_l_d_varphi
    self.B20_residual = np.sqrt(trapz(np.append(temp,temp[0]), varphi_ext) * normalizer) / B0
