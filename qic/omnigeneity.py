...
def compute_eps_eff(stel, order = 'r0', N_lam = 100, plot = False, info = None, ref_R_Bbar = None, check_geo_correction = False, include_Y3 = False, mask = None):
    ...
    if order == 'r0':
        ...
        integ_lambda = trapz(integrand_lambda * lam_array, lam_array)

        ############################
        # COMPUTE GEOMETRIC FACTOR #
        ############################
        # # G_hat needs to be computed
        # G_hat_sq = trapz(X1c**2+X1s**2+Y1c**2+Y1s**2, varphi)
        ...
        def G_hat_sq_int():
            ...
            G_hat_num = np.sum([trapz(nablapsi_chi(chi)/B0, varphi) for chi in chi_grid])/N_chi
            
            # Normalisation flux average
            norm = trapz(1/B0/B0, varphi)
        ...
        ###################
        # Compute integral over λ
        integ_lambda_1 = trapz(E_1 * lam_array, lam_array)
        integ_lambda_2 = trapz(E_2 * lam_array, lam_array)
        ...
        def G_hat_sq_int(check_correction = False):
            ...
            if check_correction:
                ...
                D1 = np.sum([trapz(T1(chi)/B0, varphi) for chi in chi_grid])/N_chi
                D2 = np.sum([trapz(T2(chi)/B0 - 2*T1(chi)*B1n(chi)/B0**2, varphi) for chi in chi_grid])/N_chi # we expect it to vanish?
                D3 = np.sum([trapz(T3(chi)/B0 - 2*T2(chi)*B1n(chi)/B0**2 + T1(chi)/B0*(3*B1n(chi)**2/B0**2-2*B2n(chi)/B0), varphi)\
                            for chi in chi_grid])/N_chi
                
                # Normalisation : L = ∫ (1/B^2) dφ
                L0 = trapz(1/B0/B0, varphi) # Leading order normalisation like before
                # L1 vanishes due to parity upon flux surface integration
                L2 = np.sum([trapz(1/B0**2*(3*B1n(chi)**2/B0**2-2*B2n(chi)/B0), varphi) for chi in chi_grid])/N_chi
        ...
        if not info is None:
            ...
            E_all["eps_eff_3_2_r2_drift"] = trapz(E_2_drift * lam_array, lam_array) * fac_eps_eff
            E_all["eps_eff_3_2_r2_resonant"] = trapz(E_2_resonant * lam_array, lam_array) * fac_eps_eff
            E_all["eps_eff_3_2_r2_I"] = trapz(E_2_I * lam_array, lam_array) * fac_eps_eff
...
def compute_eps_eff_anal(stel, r = 0.1, alpha = 0.0, N_lam = 100, verbose = False, ref_R_Bbar = None, info = None):
    ...
    with tqdm(desc = "Computing...", total = N_r) as pbar:
        for j_r, r_val in enumerate(r):
            for j_a, alpha_val in enumerate(alpha):
                ...
                ###################
                # Compute integral over λ
                int_E_arr[j_r, j_a] = trapz(E_arr * lam_array, x = lam_array)
    ...
    res_int = np.array([trapz(np.append(int_E_arr[j_r,:],int_E_arr[j_r,0]), x = np.append(alpha, 2*np.pi))/(np.pi) for j_r in range(N_r)])
