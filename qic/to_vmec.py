...
def to_vmec(self, filename, r=0.1, params=dict(), ntheta=20, ntorMax=14):
    ...
    # Get info about current installation of pyQIC 
    remote_url, branch, commit = get_qic_info()

    # Write to VMEC file
    file_object = open(filename,"w+")
    file_object.write(f"! This &INDATA namelist was generated by pyQIC: {remote_url}\n")
    file_object.write(f"! The current branch is {branch} and commit {commit}\n")
    ...
    if self.omn:
        file_object.write('! Near-axis parameters:  radius r = '+str(r)+', dbar_in = '+str(isinstance(self.d_over_curvature_in,dict))+\
                          ', d_in = '+str(isinstance(self.d_in,dict)) + '\n')
        file_object.write('! nphi = '+str(self.nphi)+', order = '+self.order+', sigma0 = '+str(self.sigma0)+', I2 = '+str(self.I2)+', Bbar = '+str(np.mean(self.Bbar))+'\n')
        file_object.write('! Frenet = '+str(self.frenet)+', differentiation scheme = '+str(self.diff_order)+'\n')
    else:
