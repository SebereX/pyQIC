...
def drift_int(stel, varphi, B, BxdBdotdpsi, BxdBdotdalpha, Bmax_in = None, Bmin_in = None, N_k = 1000, verbose=0, alpha_array = [0.0], name = [], k_chib = False, scale_wa = False):
    ...
    lB02r = trapz(np.ones(stel.nphi), x=stel.varphi)/trapz(1/stel.B0/stel.B0, x=stel.varphi)
    mu0 = 4*np.pi*1e-7
    ...
    if scale_wa == True:
        G0 = stel.G0
        norm = trapz(np.ones(stel.nphi), stel.varphi)
        one_over_B0_squared_avrg = trapz(1/stel.B0/stel.B0, stel.varphi)/norm
        one_over_B0_avrg = trapz(1/stel.B0, stel.varphi)/norm
        A_ratio = 10
...
def maxj_at_bottom(self): 
    ...
    def define_edge_flux_nae(A_ratio):
        G0 = self.G0
        norm = trapz(np.ones(self.nphi), self.varphi)
        one_over_B0_squared_avrg = trapz(1/self.B0/self.B0, self.varphi)/norm
        one_over_B0_avrg = trapz(1/self.B0, self.varphi)/norm
