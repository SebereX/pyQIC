            ...
            for spsi in [-1, 1]:
                for nfp in range(1, 4):
                    for j in range(3):
                        ...
                        nphi = int(np.random.rand() * 10 + 15)
                        if np.mod(nphi, 2) == 0:
                            nphi += 1
                        etabar = (np.random.rand() - 0.5) * 4
                        stel = Qic(Raxis={"type": "fourier", "input_value": {"cos": [R0], "sin": []}},
                                   Zaxis={"type": "fourier", "input_value": {"cos": [], "sin": [0]}},
                                   nfp=nfp, nphi=nphi, sG=sG, spsi=spsi,
                                   B0={"type": "scalar", "input_value": B0},
                                   d ={"type": "scalar", "input_value": etabar},
                                   omn = False)
                        val = sG * B0 / R0
            ...
            for spsi in [-1, 1]:
                for config in [1, 2, 3, 4, 5, 6, "QI NFP1 r2", "QI NFP2 r2"]:
                    ...
                    if config in ["QI NFP1 r2", "QI NFP2 r2"]:
                        ...
                        B0_vals = np.array(s.B0) * np.random.uniform(0.97, 1.03, len(s.B0))
                        s = Qic.from_paper(config, sG=sG, spsi= spsi, B0={"type": "grid", "input_value": B0_vals}, nphi=nphi, order = 'r3')
                    else:
                        B0 = np.random.rand() * 0.4 + 0.8
                        s = Qic.from_paper(config, sG=sG, spsi= spsi, B0={"type": "scalar", "input_value": B0}, nphi=nphi, order = 'r3')
            ...
            for spsi in [-1, 1]:
                for nfp in range(1, 4):
                    for j in range(3):
                        ...
                        s = Qic(Raxis={"type": "fourier", "input_value": {"cos": [R0], "sin": []}},
                                   Zaxis={"type": "fourier", "input_value": {"cos": [], "sin": [0]}},
                                   nfp=nfp, nphi=nphi, sG=sG, spsi=spsi,
                                   B0={"type": "scalar", "input_value": B0}, I2=1.e-8, order='r2')
...
class CylindricalCartesianTensorsTests(unittest.TestCase):
    ...
    def test_cylindrical_cartesian_tensors(self):
        ...
        stel = Qic(Raxis={"type": "fourier", "input_value": {"cos": [Rmajor, 0.], "sin": [0, 0.]}},
                   Zaxis={"type": "fourier", "input_value": {"cos": [0, 0.], "sin": [0, 0.]}},
                   d={"type": "scalar", "input_value": 1.0},
                   B0={"type": "scalar", "input_value": B0},
                   nphi=151, omn = False)
        ...
        stel = Qic.from_paper("QI NFP2 r2", nphi=251, X2c={"type": "fourier", "input_value": {"cos": [], "sin": [10.0]}}, \
                              X2s={"type": "fourier", "input_value": {"cos": [10.0], "sin": []}})
        dBdx_cylindrical = stel.grad_B_tensor_cylindrical
