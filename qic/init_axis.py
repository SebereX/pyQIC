...
def init_axis(self, omn_complete = True):
    ...
    if self.Frenet:
        ...
        self.curvature = self.evaluate_input_on_grid(self.curvature_in, self.varphi)
        self.torsion = self.evaluate_input_on_grid(self.torsion_in, self.varphi)
        self.ell = self.evaluate_input_on_grid(self.ell_in, self.varphi)
        self.axis_length = self.nfp * self.ell[-1]

        # Axis symmetry
        self.lasym_axis = np.max(np.abs(np.abs(self.curvature) - np.abs(self.evaluate_input_on_grid(self.curvature_in, -self.varphi))))/np.abs(self.curvature).std() or \
                          np.max(np.abs(np.abs(self.torsion) - np.abs(self.evaluate_input_on_grid(self.torsion_in, -self.varphi))))/np.abs(self.torsion).std()
        ...
        # Evaluate B0
        B0 = self.evaluate_input_on_grid(self.B_in, self.varphi)
        Bbar = self.spsi * np.mean(self.B0)
        ...
        d_l_d_phi = np.matmul(self.d_d_phi, self.ell) 
        ...
        self.d_l_d_varphi = np.matmul(self.d_d_varphi, self.ell)
        ...
        if self.omn:
            ...
            if isinstance(self.d_over_curvature_in, dict):
                dbar = self.evaluate_input_on_grid(self.d_over_curvature, varphi) 
                d += dbar * curvature
            self.d = d
        ...
        def compute_R0_Z0_and_derivatives(Raxis, Zaxis, omn_complete):
            ...
            if Raxis["type"] == 'fourier':
                # Define arrays with axis harmonics
                rc = Raxis["input_value"]["cos"]
                rs = Raxis["input_value"]["sin"]
                zc = Zaxis["input_value"]["cos"]
                zs = Zaxis["input_value"]["sin"]
                ...
                if self.omn and omn_complete:
                    def complete_harmonics(rc, rs, zc, zs):
                        ...
                        """
                        if (not rs) or np.max(np.abs(rs)) == 0.0:
        ...
        mean_of_Z = np.sum(Z0 * d_l_d_phi, phi) * nfp / axis_length
